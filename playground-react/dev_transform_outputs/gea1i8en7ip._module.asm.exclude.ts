"use wasm";
/**
 * Matrix Cascade Function
 * Optimized to use flat arrays to prevent memory detachment and "null" values.
 */
export function matrixCascadeWasm(m1: Float64Array, m2: Float64Array, rows: i32, cols: i32): Float64Array {
    ...
    for (let r: number = 0; r < rows; r++) {
        const rowOffset: number = r * cols;
        for (let c: number = 0; c < cols; c++) {
            let sum: f64 = 0;
            for (let k: number = 0; k < cols; k++) {
                // sum += m1[r,k] * m2[k,c]
                sum += unchecked(m1[rowOffset + k] * m2[k * cols + c]);
            }
    ...
    for (let r: number = 0; r < rows; r++) {
        ...
        for (let c: number = 0; c < cols; c++) {
            let sum: f64 = 0;
            for (let k: number = 0; k < cols; k++) {
                sum += unchecked(temp[rowOffset + k] * m1[k * cols + c]);
            }
    ...
    for (let r: number = 0; r < rows; r++) {
        ...
        for (let c: number = 0; c < cols; c++) {
            let sum: f64 = 0;
            for (let k: number = 0; k < cols; k++) {
                sum += unchecked(m2[rowOffset + k] * temp[k * cols + c]);
            }
